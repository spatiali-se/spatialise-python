...
First release cut from the self-maintained source (no longer generated by Stainless).

### Features

- **batch:** add `client.batch.retrieve_job_status(job_id, batch_id=...)` and `client.batch.retrieve_patch_batch_status(patch_batch_idx, batch_id=..., job_id=...)`, surfacing the V2 `batch → job → patch-batch` observability hierarchy. The job-detail response carries per-job patch-batch progress (`total_patch_batches` / `completed_patch_batches`) plus a paginated `patch_batches` list (`next_cursor` / `has_more`). Additive — `create` and `retrieve_status` are unchanged, so v1 clients keep working by pointing `base_url` at the v1 host. (SPA-1797, SPA-1758)
- **client:** add a `version=` argument (`"v1"` default, or `"v2"`) that selects the API host, so callers target the v2 deployment without spelling out a URL. v1/v2 routing is host-based server-side, so this is purely a client-side base-URL selector; an explicit `base_url` (or the `SPATIALISE_SOIL_PREDICTION_BASE_URL` env var) still overrides it. (SPA-1835)

### Bug Fixes

- **batch:** read batch counts from the API's real wire fields (`total_tasks` / `completed_tasks` / `failed_tasks` / `pending_tasks`), which the previous models never populated. The public `total_jobs` / `completed_jobs` / `failed_jobs` / `pending_jobs` attributes are preserved as aliases (with matching `*_tasks` properties), so existing callers are unaffected. (SPA-1797)

### Chores

- Drop Stainless code generation; the SDK is now hand-maintained. Removed the generation metadata (`.stats.yml`, the Stainless release-please fork config) and the "generated by Stainless" markers; CI, the mock server, and docs now describe the hand-maintained dev loop. (SPA-1796)
