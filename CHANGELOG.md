...
- **batch:** surface per-job patch-batch progress in `client.batch.retrieve_status`. Each `Job` in `BatchRetrieveStatusResponse` now carries optional `total_patch_batches` and `completed_patch_batches` counters, so callers can show how far a job has progressed through the V2 `batch → job → patch-batch` hierarchy before its COG is ready. Additive — existing fields are unchanged, so 0.2.x callers keep working. (SPA-1797)
